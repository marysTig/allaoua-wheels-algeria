        ...
        <div className="mt-3 grid grid-cols-1 gap-1.5 sm:mt-4 sm:grid-cols-2 sm:gap-2.5">
          {specs.map((spec) => (
            <div key={spec.label} className="flex min-w-0 items-center gap-1.5 text-xs text-muted-foreground sm:gap-2 sm:text-sm">
              <spec.icon className="h-3.5 w-3.5 shrink-0 text-primary sm:h-4 sm:w-4" />
              <span className="truncate">{spec.label}</span>
            </div>
          ))}
        </div>
        {vehicle.mileage && (
          <div className="mt-2 flex items-start gap-1.5 text-xs text-muted-foreground sm:mt-3 sm:gap-2 sm:text-sm">
            <Gauge className="mt-0.5 h-3.5 w-3.5 shrink-0 text-primary sm:h-4 sm:w-4" />
            <span className="line-clamp-2">{vehicle.mileage}</span>
          </div>
        )}
        <div className="mt-auto flex flex-col gap-2 border-t border-border pt-3 sm:mt-5 sm:flex-row sm:items-center sm:justify-between sm:pt-4">
          <div>
          ...
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
             className="inline-flex items-center justify-center gap-1.5 rounded-lg bg-primary px-2 py-2 text-xs font-semibold text-primary-foreground transition-colors hover:bg-navy-deep sm:gap-2 sm:px-4 sm:py-2.5 sm:text-sm"
          >
  ...
  return (
    <footer className="bg-navy-deep py-12 text-primary-foreground">
      ...
      <div className="mx-auto mt-10 max-w-6xl border-t border-white/10 px-4 pt-6 text-center text-xs">
        <Link
          to="/admin/login"
          className="text-primary-foreground/50 transition-colors hover:text-accent focus-visible:text-accent"
          aria-label="Accéder à la connexion administrateur"
        >
